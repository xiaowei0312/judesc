...
var balanceDetailComp = {
	...
	pageSize : null,
	totalSize : null,
	detailTemplate : "{for detail in balanceDetail}<tr><td>${detail.operationTime}</td><td>${detail.dlbNum}</td><td>+${detail.distributedMoney}</td><td>${detail.distributedYlj}</td></tr>{/for}",
	init : function() {
	...
	queryData : function(a) {
		...
		$.ajax({
			type : "POST",
			url :  this.basePath + "/member/syChangeRecordAjax",
			data : {
				page : a,
				size : this.pageSize,
